#include "../include/BlockRenderer.hpp"



BlockRenderer::BlockRenderer(Level &levelMap, Play* gamer) {
    game = gamer;
    images[29] = LoadTexture("assets/images/block_energy.png");
    for(auto & line : levelMap.FrontBlocksMap) {
    ...
    for(auto & line : levelMap.BackBlocksMap) {
        for(auto & column : line.second) {
            Block temp = column.second;
            if (images.count(temp.id) == 0) {
                addBlock(temp);
...
BlockRenderer::BlockRenderer(std::vector<Block> &blocks, Play* gamer) {
    game = gamer;
    images[29] = LoadTexture("assets/images/block_energy.png");
...
void BlockRenderer::addBlock(Block temp) {
    switch (temp.id) {
        case 1:
...
void BlockRenderer::DrawBlocks(std::map<int, std::map<int, Block>> &blocks, Rectangle cameraCenter, Rectangle relativeCameraCenter, int SCALE, bool background, bool showSecret) {
    Vector2 relativePos;
...
void BlockRenderer::DrawBlockHud(const std::vector<Block> &BlocksOption, int SCALE) {
