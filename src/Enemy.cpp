...
Enemy::Enemy(float x,
             ...
             std::string namer,
             int imagescale,
             std::map<int, std::map<int, Block>> Blocks,
             int ticker,
             int i,
             int j) {
    rect.x = x;
    ...
    if (tick == 0) {
        ...
        if (name == "bee") {
            ...
            if (Blocks.count(i) > 0) {
                for (int k = j; k > 0; k--) {
                    if (Blocks[i].count(k) > 0 and !Blocks[i][k].background and
                        Blocks[i][k].rect.x + Blocks[i][k].rect.width > patrol1) {
                        border1 = Blocks[i][k];
            ...
            if (Blocks.count(i - 1) > 0) {
                for (int k = j; k > 0; k--) {
                    if (Blocks[i - 1].count(k) > 0 and !Blocks[i - 1][k].background and
                        Blocks[i - 1][k].rect.x + Blocks[i - 1][k].rect.width > patrol1) {
            ...
            vision = {rect.x + (rectImage.width + 1) * SCALE,
                      rect.y + (rectImage.height * SCALE / 2 + 1) * SCALE,
                      (float)2 * SCALE,
                      (float)2 * SCALE};
            ...
            for (int k = j; k >= 0; k--) {
                ...
                if (Blocks[i + 1].count(k - 1) and
                    Blocks[i + 1][k - 1].rect.x + Blocks[i + 1][k - 1].rect.width > border1.rect.x and
                    !Blocks[i + 1][k - 1].background) {
            ...
            while (true) {
                ...
                if (Blocks[i + 1].count(k + 1) > 0 and
                    border2.rect.x + border2.rect.width > Blocks[i + 1][k + 1].rect.x and
                    !Blocks[i + 1][k + 1].background) {
                    k++;
                    continue;
                }
            ...
            if (Blocks.count(i - 1)) {
                for (auto const &ent1 : Blocks[i - 1]) {
                    if (ent1.first > j) {
                        if (ent1.second.rect.x < patrol2 and !ent1.second.background and
                            ent1.second.rect.y + ent1.second.rect.height > border2.rect.y) {
                            patrol2 = ent1.second.rect.x;
                            break;
                        }
                    }
            ...
            if (Blocks.count(i - 1) > 0) {
                for (k = j; k >= 0; k--) {
                    if (Blocks[i - 1].count(k) > 0 and
                        Blocks[i - 1][k].rect.x + Blocks[i - 1][k].rect.width > patrol1 and
                        Blocks[i - 1][k].rect.y + Blocks[i - 1][k].rect.height > rect.y and
                        !Blocks[i - 1][k].background) {
...
void Enemy::butterfly(std::vector<Block> Blocks, Flamingo *player) {
    ...
    if (behavior == 0) {
        ...
        if (abs(player->rect.x - rect.x) + abs(player->rect.y - rect.y) <
            60 * SCALE) {  // fun-mode: Butterfly-chase, change < to >
            behavior = 1;
        }
    } else if (behavior == 1) {
        angle -= 3;
